...
$(document).ready(function () {
  ...
  $('[dir="right"]').each(function () {
    $(".pc-controls-tab").insertBefore(".pc-controls");
  })

  // open the panel (after click on the tab)
  $(".pc-controls-tab").click(function() {
    if(!$(this).hasClass("open")) {
      $(this).addClass("open");
      $(".pc-controls").css("transform", "var(--translateX-open)");
      $(".pc-controls-tab").addClass("panel-open");
    } else {
      $(this).removeClass("open");
      $(".pc-controls").css("transform", "var(--translateX-open)");
      $(".pc-controls-tab").removeClass("panel-open");
    }
  })

  // open or close section
  $(".pc-section-header").not(".disable-closing").click(function () {
    if ($(this).hasClass("close")) {
      $(this).parent().css("max-height","2000px");
      $(this).removeClass("close");
      $(this).next().removeClass("close");
      $(this).next().children().removeClass("close");
      scrollTopSection($(this).parent());
    }
    else {
      $(this).addClass("close");
      $(this).next().addClass("close");
      $(this).parent().css("max-height","30px");
    }
  });

  $(".pc-controls").click(function(e) {
    e.stopPropagation();
  })

  // close the panel (form the >> icon)
  $(".close-controller").click(function(e) {
    $(".pc-controls-tab").removeClass("open");
    $(".pc-controls").css("transform", "var(--translateX)");
    $(".pc-controls-tab").removeClass("panel-open");
  })

  // Change the panel position - LEFT (defult) or RIGHT.
  // $(".dir-controls").click(function(e) {
  //   e.stopPropagation();
  //   $(this).toggleClass("selected");
  //   if($(this).hasClass("selected")) {
  //     $(".controls-tab").insertBefore(".controls");
  //     $("[dir]").each(function() {
  //       $(this).attr("dir","right");
  //     })
  //   } else {
  //     $(".controls").insertBefore(".controls-tab");
  //     $("[dir]").each(function() {
  //       $(this).attr("dir","left");
  //     })
  //   }
  ...
  $(".pc-input-number-area [type='number']").change(function () {
    let sliderField = $(`.pc-slider[name='${$(this).attr("name")}']`);
  ...
  $(".pc-thumbnails-tooltip-item").each(function () {
...
function init() {
  var controls = `<div class="pc-controls"></div>`;
  $(".prototype-settings").append(controls);
  var structure = `<header class="pc-controls-header"><span class="main-title">${prototypeTitle}</span></header>
  <div class="pc-controller-content"></div>`;
  // info section
  var info = `<div class="pc-section">
  <div class="pc-section-header"><span>Prototype Info</span></div>
  <div class="pc-section-content info-section-content"><span>${prototypeDescription}</span></div>
  </div>`;
  var footer = `<div class="controller-footer">${chevRight}</div>`
  $(".pc-controls").append(structure);
  $(".pc-controls").append(footer);
  $(".pc-controller-content").append(info);
  var tab = `<div class="pc-controls-tab">${settingsIcon}</div>`;
  $(".prototype-settings").append(tab);
  $.each(sections, function (i, section) { // the sections array from "customSettings.js"
    createSection(section);
  });
  $(".pc-controls-field.disabled").each(function() {
    disableField($(this),true);
  })
  if(sections.length < 1) { // section is empty
    $(".pc-controller-content").addClass("only-info-content");
    $(".pc-section-header").addClass("disable-closing");
  }
}
...
function createSection(section) {
  ...
  newSection = `<div class="pc-section" section-number="${sectionNum}">
  <div class="pc-section-header"><span>${section.sectionTitle}</span></div>
  <div class="pc-section-content" number="${sectionNum}"></div>
  </div>`;
  $('.pc-controller-content').append(newSection);
...
function createSettings(field) {
  ...
  newSetting = `<div class="pc-controls-field ${disabled}" name="${field.fieldName}" call="${field.function}"><label class="pc-controls-field-label">${field.fieldLabel}</label>${content}</div>`;
  field.divider ? newSetting = newSetting + '<div class="pc-divider"></div>' : "";
  return newSetting;
}
...
function inputFieldContent(field) {
  ...
  switch (field.fieldType) {
    ...
    content = `<div class="pc-input-number-area" style="display:flex">
    <div class="pc-slider" name="${field.fieldName}" data-value=${field.value} data-min=${field.min} data-max=${field.max} data-step=${field.step} style="display:${displaySlider}"></div>
    <div class="pc-container-input-number"><input name="${field.fieldName}" type="number" min="${field.min}" max="${field.max}" step="${field.step}" suffix="${field.suffix}" value="${field.value}">
    ...
    <input class="pc-toggle-option" id="${field.fieldName}-0" value="${field.option1Value}" option="1" type="radio" name="${field.fieldName}" checked>
    <label class="labels" for="${field.fieldName}-0">${field.option1Display}</label>
    <span class="pc-toggle-tooltip-option" style="opacity: ${opacity1}">${field.option1Display}</span>
    <input class="pc-toggle-option" id="${field.fieldName}-1" value="${field.option2Value}" option="2" type="radio" name="${field.fieldName}">
    <label class="labels" for="${field.fieldName}-1">${field.option2Display}</label>
    <span class="pc-toggle-tooltip-option right" style="opacity: ${opacity2}">${field.option2Display}</span>
    ...
    for(var i = 0 ; i < field.optionsBackendList.length; i++) {
      ...
      content += `<div><input class="pc-circle-checkbox" type="radio" id="${field.fieldName}-${i}" value="${field.optionsBackendList[i]}" name="${field.fieldName}" ${checked}>
      <label for="${field.fieldName}-${i}">${field.optionsDisplayList[i]}</label>
      </div>`;
    }
    break;
    case "thumbnails":
    content = `<div class="pc-thumbnails">`
    for(var i = 0 ; i < field.optionsBackendList.length; i++) {
      i == field.defaultIndex ? selected = "selected" : selected = "";
      i == field.defaultIndex ? checked = "checked" : checked = "";
      field.labelsDisplayList[i].length > 9 ? opacity2 = 1 : opacity2 = 0;
      content += `<div class="pc-thumbnails-item">
      <input class="pc-thumbnails-input" type="radio" value="0" name="${field.fieldName}" id="${field.fieldName}-${i}" ${checked}>
      <button class="pc-thumbnails-button ${selected}" value="${field.optionsBackendList[i]}"><img src="${field.iconsDisplayList[i]}"></button>
      <label for="${field.fieldName}-${i}">${field.labelsDisplayList[i]}</label>
      <span class="pc-thumbnails-tooltip-item" count="${i+1}" style="opacity: ${opacity2}">${field.labelsDisplayList[i]}</span>
      </div>`;
    }
    content += `</div>`;
    break;
...
function initControls() {
  $(".pc-controls-field input").on("change", e => {
    e.stopPropagation();
    ...
    const theFunction = $(`.pc-controls-field[name='${name}']`).attr("call");
